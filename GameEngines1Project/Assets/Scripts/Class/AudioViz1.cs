...
public class AudioViz1 : MonoBehaviour {
    ...
    void CreateVisualisers()
    {
        ...
        for (int i = 0; i < AudioAnalyzer.frameSize; i+=2)
        {
            ...
            cube.GetComponent<Renderer>().material.color = Color.HSVToRGB(i / (float)AudioAnalyzer.frameSize, 1, 1);
            elements.Add(cube);
        }
    }
    ...
    void Update () {
        for (int i = 0; i < elements.Count; i++) {
            elements[i].transform.localScale = new Vector3(0.25f, 0.5f + AudioAnalyzer.spectrum[i] * scale, 0.25f);
        }
	}
}
